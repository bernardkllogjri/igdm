function getLoggedInUser() {
  eventEmitter.emit("getLoggedInUser");
}
...
function getUnfollowers() {
  eventEmitter.emit("getUnfollowers");
}

function unfollow(userId) {
  eventEmitter.emit("unfollow", userId);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  eventEmitter.on("chatList", (evt, chats_) => {
    if (
      !window.chats.length ||
      window.chats[0].items[0].id !== chats_[0].items[0].id
    ) {
      window.chats = chats_;
  ...
  eventEmitter.on("chat", (evt, chat_) => {
    let isNewMessage =
      !window.chat.items ||
      !window.chat.items.length ||
      !chat_.items.length ||
      window.chat.items[0].id != chat_.items[0].id ||
      getIsSeenText(chat_) != getIsSeenText(window.chat);

    if (isNewMessage && isCurrentChat(chat_) && !window.gettingOlderMessages)
      renderChat(chat_);
  });

  eventEmitter.on("olderMessages", (_, messages) => {
  ...
  tx.setAttribute(
    "style",
    "height:" + scrHeight.toString() + "px;overflow-y:hidden;"
  );
